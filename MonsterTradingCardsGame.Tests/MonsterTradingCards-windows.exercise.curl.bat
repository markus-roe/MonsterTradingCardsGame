    ...
    REM --------------------------------------------------
    echo Mandatory Feature 1: Trade Card for Coins (trade random stack card, for 6 coins)
    echo kienboec buys coins 
    curl -X POST http://localhost:10001/coins --header "Authorization: Bearer kienboec-mtcgToken"
    echo.
    echo try to trade trade without cards (admin has no cards -> should fail)
    
    curl -X POST http://localhost:10001/coins --header "Authorization: Bearer admin-mtcgToken"

    REM --------------------------------------------------
    echo Mandatory Feature 2: Merge Cards (merge two random cards from deck, if they are different element type, creates a new one, removes random old card, costs 3 coins)
    echo kienboec tries to merge cards 2 times
    curl -X POST http://localhost:10001/merge --header "Authorization: Bearer kienboec-mtcgToken"
    curl -X POST http://localhost:10001/merge --header "Authorization: Bearer kienboec-mtcgToken"
    echo.
    echo admin tries to merge cards (admin has no cards -> should fail)
    curl -X POST http://localhost:10001/merge --header "Authorization: Bearer admin-mtcgToken"
